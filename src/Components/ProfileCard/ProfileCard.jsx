...
export const ProfileCard = ({
  ...
  return (
    ...
    >
      <div
        className={`relative w-full h-full transition-transform duration-750 ${
      ...
      >
        ...
        {/* Back Side */}
        <div
          className="absolute inset-0 bg-amber-200 rounded-3xl p-6 overflow-hidden"
          style={{ transform: "rotateY(180deg)", backfaceVisibility: "hidden" }}
        >
          <img
            className="absolute top-0 left-0 z-0 blur-xs"
            style={{ transform: "rotateY(180deg)" }}
            src={ProfilePicture}
            alt="Profile Picture"
          />
          <div className="h-full flex flex-col z-10">
            <h2 className="text-2xl font-bold z-10 text-white mb-2">{Name}</h2>
            <p className="text-sm text-white font-semibold underline underline-offset-2 mb-1 z-10">
              {role}
            </p>

            <div className="flex-grow overflow-y-auto py-2 z-10">
              <p className="text-white text-sm z-10">{description}</p>
            </div>
          </div>
        </div>
      </div>
